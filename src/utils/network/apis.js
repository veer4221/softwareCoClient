export const loginAPI = `/auth/login`;
export const signUpAPI = `/user/Signup`;
export const getUserListAPI = `/user/getAllUsers`;
export const roleMenuAPI = `/roleMenu/getallRole`;
export const updateUserAPI = `/user/updateUser`;
export const getUserAPI = `/user/getUser`;
export const addProductAPI = `/ProductRoute/createProduct`;
export const getAllProductAPI = `/ProductRoute/getAllProduct`;
export const addToCartAPI = `/ProductRoute/addtocart`;
export const getallRoleAPI = `/roleMenu/getallRole`;
export const getProductAPI = `/ProductRoute/getProduct`;
export const addRoleAPI = `/roleMenu/addRole`;
export const getroleMenuAPI = `/rolemenu/getrolemenu`;
export const getRoleByIdAPI = `/rolemenu/getrole`
export const roleUpdateAPI = `/rolemenu/roleUpdate`
export const getCartAPI = `/ProductRoute/getCart`
export const removeFromCartAPI = `/ProductRoute/removeFromCart`